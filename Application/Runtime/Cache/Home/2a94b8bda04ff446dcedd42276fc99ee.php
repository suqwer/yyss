    ...
    <div class="cont_login">
      ...
      <div class="cont_forms" >
        ...
        <div class="cont_form_sign_up"> <a href="#" onclick="ocultar_login_sign_up()"><i class="material-icons">&#xE5C4;</i></a>
          ...
          <input type="text" placeholder="Email" class='emsub inpts' name="email" />
          <input type="text" placeholder="Nick Name" name="name" class="inpts"/>
          <input type="password" class='pas inpts' placeholder="Password" name="password" />
          <input type="password" class='rpas inpts' placeholder="Confirm Password" name="repassword"/>
...
<script>
    ...
    $('.pas').on('blur',function () {
          if($('.pas').val().length>5&&$('.pas').val().length<16){

          }else{
            $('.pas').val('');
            $('.pas').css('border','1px solid red');
            $('.pas').attr('placeholder','密码必须大于等于6位小于等于16位');
            return false;
          }
    });

    $('.rpas').on('blur',function () {
          if($('.pas').val()==$('.rpas').val()){
            $('.rpas').css('border','0px solid red');
          }else{
            $('.rpas').val('');
            $('.rpas').css('border','1px solid red');
            $('.rpas').attr('placeholder','两次输入不一致');
            return false;
          }
    })
